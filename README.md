...
# 선택: Gemma 4 + Ollama local worker를 쓰려면
...
### 2. 설치 및 환경 (Setup)
- **[Gemma 4 & Ollama Guide](docs/setup-gemma4-ollama.md)**: Ollama local endpoint 기준의 Gemma 4 worker 설정 가이드.
- **[Architecture Snapshot](docs/arch-snapshot.md)**: 현재 구현된 기술 스택 및 모듈 구조.
- `python3 scripts/ollama_proxy.py`: Ollama API 응답에서 `thinking` 필드를 제거하는 투명 프록시.
- `Modelfile.gemma4-gguf`: Unsloth Gemma 4 GGUF 모델을 Ollama에 등록하기 위한 설정 파일.

### 3. 프로젝트 관리 (Meta)
- **[Project Plan](plan.md)**: v2 비전 및 단계별 로드맵.
- **[Parity Status](docs/PARITY.md)**: 현재 구현된 기능 검증 리스트.
- **[Scion Operations](docs/scion-operations.md)**: Phase 7 file-backed coordination 운영 절차.

## 🛠️ Key Extensions (MCP)
- `metric-benchmark`: 성능 측정
- `llm-wiki-compiler`: 지식 구조화
- `output-compactor`: 토큰 효율화 (로그 압축)
- `scion-coordination`: `.scion/shared` 기반 file-backed 병렬 협업 및 file claim 조정
